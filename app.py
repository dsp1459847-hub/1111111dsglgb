...
st.set_page_config(layout="wide", page_title="MAYA AI: QUADRUPLE FIX v2")
...
# --- 8 Golden Ank Logic (FIXED) ---
def get_golden_8(df, t_date):
    hist = df[df['DATE'] < pd.to_datetime(t_date)].tail(30)
    all_vals = []
    # DS, GL, GD teeno se values collect karna
    for c in ['DS','GL','GD']: 
        vals = [x for x in hist[c].tolist() if x and x != "XX"]
        all_vals.extend(vals) # Yahan bracket fix kar diya gaya hai
